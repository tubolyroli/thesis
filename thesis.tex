...
\textbf{AI scoring.} Each week-library observation in the GitHub panel carries an AI commit score — a continuous measure of the probability that the importing commit was generated with LLM assistance, based on commit message and code-diff characteristics. This score is aggregated to the package level as a commit-weighted average AI score over the first 52 weeks post-release (\texttt{avg\_ai\_score\_52wk}). Packages above the median of this score are classified as High AI; those below are classified as Low AI. This variable is used exclusively in the exploratory mechanism test (Section~\ref{sec:methods-mechanism}).
...
\begin{itemize}
    ...
    \item Bandwidth: MSE-optimal, data-driven selection.
...
$$\hat{\tau}^{DiD} = \hat{\tau}_{2021} - \frac{1}{3}\sum_{t \in \{2018,2019,2020\}} \hat{\tau}_t$$
...
The stability of estimates is evaluated across a range of bandwidths $h \in \{13, 15, 20, 26, 30, 40\}$ weeks. The single-year 2021 RDD showed sensitivity to very narrow bandwidths, and the data-driven MSE-optimal bandwidth selection (\texttt{rdrobust}) occasionally failed to converge or produced overly wide windows. This sensitivity provided the empirical justification for the Diff-in-RDD strategy, which remains stable and significant across these same bandwidth choices by using previous years (2018--2020) as a counterfactual for seasonal release patterns.
...
Discontinuities in predetermined covariates are tested, including library licenses, the presence of specific category tags (e.g., ``AI,'' ``Data Science''), and whether the library was already mirrored on GitHub at the time of its PyPI release. The balance of these covariates across the cutoff ensures that libraries released just before versus just after the cutoff are comparable in terms of their pre-treatment characteristics and intended purpose.

\subsection{Alternative Polynomial Orders}\label{subsec:rob-polynomial}
While local linear regression ($p=1$) is the standard best practice to avoid over-fitting at the boundary \citep{gelman2019high}, results are verified using local quadratic ($p=2$) specifications. This check confirms that the detected discontinuity at the cutoff is not a result of mismodeling a non-linear underlying trend in library diffusion.

\subsection{Outcome Scale: Inverse Hyperbolic Sine}\label{subsec:rob-ihs}
Given the heavily skewed distribution of PyPI downloads and GitHub imports (which often contain many zeros in the Broad tier), the Inverse Hyperbolic Sine (IHS) transformation is employed as a robustness check. IHS behaves similarly to a log transformation, $f(y) = \ln(y + \sqrt{y^2 + 1})$, but is defined at zero, allowing the full sample of libraries to be included while mitigating the influence of extreme outliers that could skew mean-based estimates.

\section{Permutation Inference}\label{sec:methods-permutation}
A non-parametric permutation test provides a distribution-free assessment of whether the discontinuity at the September 2021 cutoff is unusual relative to what would be observed at arbitrary dates. Following the moving-cutoff logic common in the causal inference literature (e.g., Dell, 2010), the primary RDD model is re-estimated using 157 alternative placebo Mondays between January 2019 and January 2022.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Statistics computed on the full 2021 analysis sample within the MSE-optimal bandwidth of the cutoff, excluding the 9-week donut. Downloads and imports are in levels (not logged). AI exposure scores are available only for the GitHub-matched subsample with non-zero commit activity.
\end{tablenotes}
\end{table}
...
Table~\ref{tab:mainrdd} reports the baseline RDD estimates for the 2021 cohort, estimated using \texttt{rdrobust} with bias-corrected local linear regression, triangular kernel, and MSE-optimal bandwidth. The donut exclusion (weeks $-8$ to $0$) is applied.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Bias-corrected robust estimates via \texttt{rdrobust}. Triangular kernel, MSE-optimal bandwidth, 9-week asymmetric donut. Post-AI downloads are cumulative from November 28, 2022 onward.
\end{tablenotes}
\end{table}
...
\caption{Bandwidth sensitivity of the Diff-in-RDD excess jump (Successful tier, min.\ 500 downloads at 26 weeks). Points are bias-corrected estimates; vertical bars are 95\% confidence intervals from year-by-week cluster-robust standard errors. The estimate is negative and stable across $h \ge 13$.}\label{fig:bw-sensitivity}
\end{figure}
...
The conceptual foundation draws on \citet{doshi2024generative}, who demonstrate that generative AI improves individual creative quality while reducing collective diversity through convergence. The parallel to this thesis is structural: AI steers developers toward convergent tool choices — specifically, tools established before the knowledge cutoff. Individual developers gain productivity (individual enhancement), but the aggregate effect is a concentration of adoption around the pre-cutoff library set (collective narrowing).
...
Finally, \textbf{retrieval-augmented generation (RAG) and web search} capabilities in modern LLMs could in principle erode the knowledge asymmetry. However, web search capabilities were not widely deployed in the mass-use products (ChatGPT, GitHub Copilot) during 2022--2023. Where available, they required explicit user activation and incurred higher latency costs that discourage routine use. The persistent and growing implementation gap through early 2026 suggests that retrieval has not materially offset the knowledge asymmetry created by the training cutoff.
...
In accordance with the BCE TDK competition rules (Section IV.1.b), the following declaration describes all uses of artificial intelligence tools during the preparation of this thesis. Three AI tools were used, each restricted to minor subtasks as defined by the competition rules.
