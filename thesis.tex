...
\author{Roland Tuboly\\[6pt]
\small Corvinus University of Budapest\\
\small School of Data Science, MSc Program\\[6pt]
\small Supervisor: Johannes Wachs}
\date{2026}
...
\begin{itemize}
    ...
    \item Post-AI cumulative downloads: (From Nov 2022) Directly attributable to LLM-mediated discovery.
    \item 26-week cumulative downloads: Used for the barrier-to-entry analysis.
\end{itemize}
...
The all-time estimate (+27.9 log points) is approximately twice the corresponding PyPI post-AI estimate. The amplification of the effect in code repositories is consistent with LLM-steered code generation as the operative channel. Figure~\ref{fig:horizon-coef} shows that the estimated discontinuity grows monotonically from the 52-week horizon through the all-time horizon, consistent with the activation hypothesis: the gap is dormant at release and emerges only after LLMs become a mass interface.
...
Neither interaction is statistically significant. This null result is interpreted as consistent with the Knowledge Wall operating as a broad ecosystem effect rather than a targeted penalty.
