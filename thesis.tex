...
The identification strategy is a sharp regression discontinuity design (RDD) with a nine-week donut exclusion for libraries whose training data inclusion is ambiguous. The main causal estimate comes from a difference-in-discontinuities (Diff-in-RDD) specification that differences the 2021 discontinuity against placebos at the same timeframe in 2018--2020. Inference in this study uses bias-corrected local linear estimation with a fixed bandwidth of $h = 13$ weeks and cluster-robust standard errors.

The analysis reveals three findings. First, post-cutoff libraries suffer a significant diffusion penalty once LLMs become widely available: before ChatGPT, libraries on either side of the cutoff differed by roughly 28\% in weekly downloads (a seasonal baseline); by early 2026, pre-cutoff libraries average approximately five times more weekly downloads, and the gap continues to widen. The baseline-adjusted Diff-in-RDD estimate confirms that this suppression is statistically significant ($p = 0.002$) among libraries that achieved at least 500 downloads in their first 26 weeks; post-cutoff libraries were more successful before ChatGPT, making the reversal conservative. Second, the penalty is roughly twice as large in GitHub code imports as in PyPI downloads, consistent with LLM-steered code generation as the operative channel. Third, the gap was absent in all placebo years and emerged only after November 2022.

The results suggest that LLM training cutoffs function as a structural barrier to entry, a ``Knowledge Wall,'' that redirects adoption toward already-known tools. This study has limitations, which include imperfect proxy measurement, an underpowered mechanism test, and local identification.
...
This transformation raises a structural question: if LLMs function as a mediator between developers and the software library ecosystem, the temporal boundary of their training data creates a systematic asymmetry in developer access to knowledge. Libraries released before a cutoff are visible; those after are not. This paper provides an empirical answer in case of the Python package ecosystem.

\section{The Mechanism}\label{sec:intro-mechanism}
The mechanism operates through a specific property of pretrained language models: their parametric knowledge is fixed when they are being trained. A model trained on data through September 2021 has no knowledge of a library first released in November 2021, regardless of that library's quality, functionality, or subsequent adoption. When a developer asks a model like this to recommend a library for a given task, or when the model generates code to accomplish a task, the post-cutoff library is simply not part of the candidate set, but the pre-cutoff library, even an older or less capable one, is.

This asymmetry is not hypothetical: the GPT-3.5 and GPT-4 model families, which powered the majority of AI coding assistant usage in 2022--2024, share a documented training data cutoff of September 2021 (OpenAI, 2023). A Python library released before October, 2021 was a candidate for inclusion in these models' knowledge; a library released in September, 2021, probably was not. If LLM recommendations shape adoption, this discrete boundary should produce a detectable discontinuity in the diffusion trajectories of libraries released just before versus just after the mentioned cutoff date.

The proposed causal mechanism says that training cutoffs result in differential knowledge representation, which in turn leads to variations in LLM recommendation frequencies. These recommendations subsequently influence rates of developer adoption. While each stage of this sequence is plausible, the empirical challenge lies in determining whether the cumulative effect is robust enough to be interpreted as measurable population-level shifts in observational data.

\section{The Research Gap}\label{sec:intro-gap}
The impact of AI systems on collective knowledge and the diversification of creative output is a subject of recent academic literature. \citet{doshi2024generative} demonstrate in a controlled experiment that generative AI improves the average quality of individual creative output while simultaneously reducing the diversity of outputs across participants. Individuals assisted by AI converge toward similar high-quality solutions, compressing collective exploration. The implication is that AI systems which improve individual productivity may reduce the aggregate variety of ideas, products, and approaches that a community of people generate.

The software ecosystem offers a tractable empirical setting in which to test whether the same logic applies for tool adoption. If LLMs steer developers toward pre-cutoff libraries, the collective variety of tools in active use may narrow over time, not because post-cutoff libraries are worse, but because they are unknown to the intermediary through which developers now routinely access the ecosystem - LLMs. This would represent a Knowledge Wall: a concentration mechanism that is basically invisible to any individual but operates at the aggregate level to shape the distribution of libraries.

The empirical literature on technology diffusion and platform economics has documented how recommendation systems, network effects, and algorithmic curation influence product adoption \citep{bass1969new, rogers2003diffusion, kleinberg2021algorithmic, bommasani2022picking}, but has not yet addressed the role of LLM training boundaries as a specific mechanism. The software ecosystem literature documents factors shaping library survival and adoption \citep{borges2016understanding, borges2018whats}, but predates the mass deployment of AI coding assistants. This paper sits at the intersection of these two bodies of work, contributing an empirically identified estimate of a mechanism specific to the current moment.

\section{Research Setting}\label{sec:intro-setting}
The Python Package Index (PyPI) provides an unusually well-suited setting for this investigation. Python is the dominant language for data science, machine learning, and scientific computing \citep{stackoverflow2024survey}, precisely the domains in which AI-assisted coding is most prevalent \citep{daniotti2025who}. PyPI records weekly download counts for every registered package, enabling the construction of library diffusion curves that begin at the week of first observed download. GitHub complements this dataset with implementation-level evidence: a panel of weekly import counts per library, with an additional layer of commit-level AI scoring that indicates whether each importing commit was generated with LLM assistance \citep{daniotti2025who}.

The combination of these sources allows the question to be asked at two levels. First, does the training cutoff affect general developer adoption, which is measured by PyPI downloads? Second, does it specifically affect AI-mediated code generation, as measured by the GitHub imports that are attributable to AI-assisted commits? If the effect is larger in the latter, this is evidence that the channel runs through LLM-generated code rather than general attention or discoverability. A documented knowledge cutoff (September, 2021) enables clean identification through a Regression Discontinuity Design.
...
The strength of this setting is the natural experiment it provides. The September 2021 training data cutoff was determined by OpenAI's data collection schedule, exogenous to any individual library's release decision. We can therefore use the exogenously defined and applied cutoff date to consider libraries released pre- and post-cutoff as assigned to treatment and control as-if randomly. This in turn allows us to estimate the causal effect of inclusion into AI training data on widespread use.

The primary estimation strategy is a Difference-in-Discontinuities (Diff-in-RDD) design \citep{grembi2016fiscal}. The 2021 cohort is compared against 2018--2020 placebo groups to control for seasonal patterns. An asymmetric donut exclusion handles data pipeline lags, and success tiers are defined based on pre-ChatGPT performance to avoid selection bias. Figure~\ref{fig:timeline} illustrates the design.
...
The main estimate, from a baseline-adjusted Difference-in-Discontinuities design using 2018--2020 as placebo years, finds that libraries narrowly included in the training data of the first ChatGPT average approximately three times more weekly PyPI downloads in the post-ChatGPT period than narrowly excluded libraries ($p = 0.002$). Before ChatGPT, the two groups differed by only about 28\% (reflecting seasonal release timing); by early 2026, the gap has widened to roughly 500\%, with no sign of closing. The suppression is concentrated among libraries that demonstrated pre-ChatGPT quality, not driven by low-quality noise at the boundary. It is substantially larger in GitHub code imports than in PyPI downloads, consistent with the LLM code generation channel. The phenomenon was latent before November 2022, activating only with the mass adoption of ChatGPT, which transformed the training cutoff into a binding constraint on developer practices.

\section{Contribution}\label{sec:intro-contribution}
This paper provides, to the best of my knowledge, the first quasi-experimental estimate of the causal effect of LLM training data boundaries on the diffusion of new software packages. The identification strategy exploits a documented institutional feature (the dated GPT-3.5/GPT-4 training cutoff) to generate a discontinuity in expected knowledge representation. It also introduces a two-outcome research design that distinguishes general developer adoption (PyPI) from implementation-level usage (GitHub), enabling the identification of a channel effect not available from download data alone. Furthermore, it provides empirical evidence for the theoretical framework, articulated by \citet{doshi2024generative} in the context of creative output, that AI systems which improve individual productivity may simultaneously narrow collective adoption diversity, here concretized in the specific mechanism of training data boundaries.
...
The following chapter maps the paper within four distinct scholarly domains. Section~\ref{sec:lit-creativity} covers the emerging literature on AI and collective diversity. Section~\ref{sec:lit-monoculture} reviews research on algorithmic monoculture and platform concentration. Section~\ref{sec:lit-diffusion} develops the theoretical context from technology diffusion and innovation ecology. Section~\ref{sec:lit-productivity} surveys empirical work on LLMs in economic activity. Section~\ref{sec:lit-boundaries} covers technical work on LLM knowledge boundaries and their practical limits. Section~\ref{sec:lit-contribution} states precisely what this paper adds.

\section{AI, Creativity, and Collective Diversity}\label{sec:lit-creativity}

The central theoretical concern motivating this paper is that tools which improve individual productivity may simultaneously narrow the collective variety of outputs they help produce. The most direct empirical grounding for this concern is \citet{doshi2024generative}, who conduct a preregistered online experiment in which participants wrote short stories with or without access to a generative AI system. Access to AI increased average story quality, consistent with individual productivity gains. The same access, however, induced a narrowing of the output variance; stories generated with AI support were significantly more uniform than the purely human-authored ones. The authors interpret this as a social dilemma: each individual is better off with AI assistance; yet the aggregate range of novel content produced is narrower. The mechanism is convergence: AI suggestions push participants toward similar high-quality solutions, compressing the exploratory variety that human writers would otherwise generate.

The analogy to software adoption is structural, not empirical. Doshi and Hauser study creative text; this paper examines tool selection. The logic, however, is the same: when developers query an LLM, the model draws on parametric knowledge to suggest options. If many developers query the same model, they receive correlated suggestions concentrated on the libraries  represented in training data. The collective effect is a narrowing of the active tool set, not because post-cutoff libraries are worse, but because the information broker cannot consider them. Doshi and Hauser (2024) serve as a motivation and a structural analogy for this paper; they are not directly comparable empirical evidence.

Related experimental evidence strengthens the micro-level plausibility of this mechanism. \citet{anderson2024homogenization} examine creative ideation in an experiment where participants used either ChatGPT or an alternative support tool. Users of ChatGPT generated more detailed ideas but conceptually more overlapping ones than users of the alternative tool. The homogenization was group-level rather than individual: different users received similar suggestions from the same model, converging on similar ideas even when working independently. This group-level convergence is precisely the mechanism that would, if replicated in the domain of library discovery, produce the population-level adoption pattern this paper identifies. A closely related phenomenon is described by \citet{wu2025generative} as 'generative monoculture.' Their research shows that LLM outputs exhibit narrower diversity than their underlying training sets. Rather than blaming the data composition, they trace this effect to reinforcement learning from human feedback (RLHF). Consequently, even with updated training data, this pressure toward uniformity would likely persist, as the alignment process is designed to favor the most probable outputs.

\citet{raghavan2024competition} provides a theoretical counterpoint using a game-theoretic model, showing that competition among AI providers selects for diverse outputs at equilibrium, potentially mitigating the monoculture mentioned by Wu et al. The empirical implication is that homogenization pressure may diminish as the ecosystem of available models becomes more heterogeneous. The competitive equilibrium result applies, however, to markets with genuinely differentiated providers. It does not address the case studied in this paper, where multiple dominant models (GPT-3.5, GPT-4, Copilot) shared a common training cutoff during the period of mass deployment and therefore a common knowledge boundary. Competition may moderate monoculture in outputs; it does not eliminate a shared temporal constraint in training data.

\section{Algorithmic Monoculture and Platform Concentration}\label{sec:lit-monoculture}

The broader literature on algorithmic monoculture addresses the systemic risks that arise when shared decision systems are deployed at scale. \citet{kleinberg2021algorithmic} formalize the core argument in a probabilistic framework: when multiple decision-makers adopt the same algorithm, the correlation in their decisions reduces collective decision quality even if the algorithm is individually optimal, because correlated errors cannot be diversified away. Applied to library discovery, when many developers prompt the same LLM, their exploration of the ecosystem is correlated, concentrating adoption on the same subset of pre-cutoff tools and reducing the effective diversity of the solutions they produce.

\citet{bommasani2022picking} extend this framework to foundation models\footnote{Foundation models are large-scale models trained on broad data and adapted to a wide range of downstream tasks. Examples include GPT-4, Claude, and Gemini. The term was introduced by \citet{bommasani2022foundation}.} specifically. Testing whether shared model components make outcomes similar across decision-makers in vision and language tasks, they find that sharing training data reliably increases outcome homogenization at the individual level, with effects that exceed group-level averages. Their result is directly relevant to the mechanism proposed in this paper: when developers share the same base model, the correlation in library suggestions is an empirical prediction of their framework, not a speculative concern.
...
\citet{daniotti2025who} provide large-scale observational evidence on the diffusion of AI coding tools in the Python ecosystem, training a neural classifier to identify AI-generated functions across 80 million GitHub commits by 200{,}000 developers between 2019 and 2024. Their adoption trajectory documents sharp increases in AI-generated code coinciding with the launches of GitHub Copilot, ChatGPT, and GPT-4, with US developers reaching approximately 30\% AI-generated code by December 2024. This figure establishes the quantitative scale of LLM mediation in the Python ecosystem that the Knowledge Wall mechanism requires: if roughly a third of Python functions in active repositories are now AI-generated, the training boundary determines what a substantial share of production code can reference. Daniotti et al.\ also find that AI use is associated with increased exploration of new libraries, including libraries novel to the ecosystem. This result provides a useful boundary condition for the present analysis: aggregate library exploration expands under AI assistance, but the training boundary constrains which libraries enter that expanded set. The Knowledge Wall does not suppress total experimentation; it biases the direction of experimentation toward tools already visible to the model.
...
estimated using observations within a bandwidth $h$ ($|X_i| \leq h$). The primary specification includes pre-AI 52-week cumulative downloads ($\log(1 + B_i)$) as a covariate via the \texttt{covs} parameter of \texttt{rdrobust}. This ANCOVA-style adjustment absorbs cross-sectional variation in library scale and improves precision without altering the local randomization logic. The covariate is omitted when the outcome is 52-week downloads itself, to avoid circularity. Estimates use the \texttt{rdrobust} package \citep{calonico2014robust, calonico2020rdrobust} with:
\begin{itemize}
...
where $B_i$ is the library's pre-AI 52-week cumulative download count, $D_{it} = \mathbb{1}(X_{it} \geq 0)$ is the post-cutoff indicator, $\mathbb{1}_{2021}$ is an indicator for the 2021 cohort, and $g(\cdot)$ contains the running variable $X_{it}$ and its full set of interactions with $D_{it}$ and $\mathbb{1}_{2021}$, allowing separate slopes on each side of the cutoff in each cohort. The baseline covariate $\log(1 + B_i)$ enters additively: it controls for pre-existing differences in library scale without interacting with the treatment or year indicators, reducing residual variance and absorbing cross-sectional heterogeneity. When the outcome variable is $B_i$ itself (52-week downloads), the covariate is omitted to avoid circularity. Triangular kernel weights $w_i = 1 - |X_i|/h$ are applied, with bandwidth $h = 13$ weeks. The coefficient of interest is $\beta_3$: the excess discontinuity in the 2021 cohort relative to the placebo average.
...
The stability of estimates is evaluated across a range of bandwidths $h \in \{10, 13, 18, 26, 39, 52\}$ weeks. The single-year 2021 RDD showed sensitivity to bandwidth choice, and the MSE-optimal bandwidth selection in \texttt{rdrobust} failed numerically for the large-$N$ PyPI specifications. This sensitivity provided the empirical justification for the Diff-in-RDD strategy, which remains stable and significant across these same bandwidth choices by using previous years (2018--2020) as a counterfactual for seasonal release patterns.

\subsection{Donut Specification}\label{subsec:rob-donut}
The primary design uses an asymmetric 9-week donut (excluding the period from early August to September 27, 2021). This is motivated by the substantive uncertainty of whether libraries released in the final weeks before the cutoff were successfully scraped and processed into the model's training set. To ensure this specific exclusion does not drive the result, a standard symmetric $\pm 4$-week donut is also reported. The effect remains robust (e.g., $Estimate = -4.35, p < 0.001$ for the Successful tier), suggesting the Knowledge Wall is not an artifact of the specific exclusion window.
...
Figure~\ref{fig:binscatter-pypi} visualizes this pattern: the positive raw jump is visible at both the 26-week and 52-week horizons, consistent with a well-documented seasonal pattern in developer software adoption. The positive discontinuity in the Broad tier disappears entirely in the Successful subsample ($-0.195$, $p = 0.950$). This is the first signal of the barrier-to-entry mechanism.
...
In descriptive terms, libraries narrowly included in the training data average roughly three times more weekly PyPI downloads than narrowly excluded libraries in the post-ChatGPT period, a ratio that grows to approximately 5:1 by early 2026, compared to a modest pre-ChatGPT baseline difference of about 28\%.\footnote{These descriptive ratios compare mean weekly downloads for libraries released in the four weeks before the donut to those released in the four weeks after the cutoff; the Diff-in-RDD estimates control for seasonal patterns via the placebo-year baseline.} The Diff-in-RDD formalizes this: the baseline-adjusted excess jump is $-7.3$ ($p = 0.002$) for the Successful tier and $-7.3$ ($p = 0.001$) for the Broad tier (Panel~B of Table~\ref{tab:diffrdd}). The unadjusted estimates are larger in magnitude ($-12.8$ and $-9.1$, Panel~C), as expected: adding $\log(1 + \text{52-week downloads})$ as a covariate absorbs pre-existing level differences and halves the standard errors, yielding a cleaner and more precise estimate of the ChatGPT-specific penalty.

The 52-week outcome provides a pre-ChatGPT baseline. For nearly all libraries within the $h = 13$ bandwidth, the 52-week accumulation window closes before November 2022; libraries at $\text{dist} = +1$ (the nearest post-cutoff observation) have their 52-week window end in early October 2022, still eight weeks before ChatGPT. The 52-week Diff-in-RDD for the Successful tier ($-3.7$, $p = 0.026$; Panel~A of Table~\ref{tab:diffrdd}) therefore reflects a pre-existing seasonal difference: the typical autumn advantage for post-September libraries was smaller in 2021 than in placebo years, even before LLMs became widely available. The ANCOVA specification in Panel~B directly absorbs this pre-existing variation, making the separate netting calculation ($-12.8$ minus $-3.7$) unnecessary; the adjusted estimate of $-7.3$ already reflects the ChatGPT-specific penalty net of baseline differences.
...
Figure~\ref{fig:long-horizon} provides the key visual test of the activation hypothesis. The two cohorts differ modestly before November 2022 (reflecting seasonal timing), after which the pre-cutoff advantage widens sharply and persistently. By early 2026, the ratio is approximately 5:1, with the gap still growing. The widening after ChatGPT, and its absence before, constitutes descriptive evidence that the Knowledge Wall activated at mass LLM deployment rather than at the training cutoff itself.
...
The selection pattern is also visible in implementation survival rates. Among GitHub-matched Successful-tier libraries in the near-cutoff window, 44.1\% of pre-cutoff and 42.5\% of post-cutoff libraries accumulate at least 10 post-AI all-time imports; 14.3\% of pre-cutoff versus 12.6\% of post-cutoff libraries accumulate at least 100 imports. These raw differences are small in absolute terms and are not adjusted for running variable continuity. They are, however, directionally consistent with the RDD estimates: post-cutoff libraries are less likely to achieve implementation traction at every threshold, despite being more selected for pre-AI quality. The RDD estimates ($+26.8$ for post-AI imports, Table~\ref{tab:github-rdd}) capture the sharp local discontinuity at the boundary that these sample-average rates smooth over.

\section{Mechanism Moderation Test}\label{sec:results-moderation}
An exploratory test splits the sample by AI exposure intensity (above vs. below median of the commit-level AI score) and estimates whether the Knowledge Wall penalty differs between the two groups. Neither interaction is statistically significant ($p = 0.179$ for 52-week downloads, $p = 0.505$ for baseline-adjusted post-AI downloads; $N = 1{,}303$). The test is almost certainly underpowered with approximately 650 observations per group. The null result cannot distinguish between a broad ecosystem-wide penalty and a targeted effect that the test lacks power to detect. Full specification and tables are reported in Appendix~\ref{app:mechanism}.

\section{Robustness Checks}\label{sec:results-robustness}

The main Diff-in-RDD estimates are stable across bandwidths $h \in \{13, 15, 18, 26\}$ weeks in both sign and approximate magnitude (Appendix~\ref{app:bandwidth}). Replacing the asymmetric 9-week donut with a symmetric $\pm 4$-week exclusion produces qualitatively consistent results. No significant discontinuity in the density of release dates at the cutoff is detected by the McCrary test (Figure~\ref{fig:density}), providing no evidence for sorting around the boundary. A moving-cutoff permutation test confirms that the raw RDD estimate is not unusual relative to arbitrary placebo dates ($p = 0.344$), reinforcing the need for the Diff-in-RDD design (Appendix~\ref{app:permutation}). The GitHub implementation estimates are more sensitive to bandwidth choice than the PyPI Diff-in-RDD and are therefore treated as descriptive evidence consistent with the channel interpretation, not as independently robustified causal estimates.
...
The core result is a baseline-adjusted Diff-in-RDD penalty of $-7.3$ ($p = 0.002$) in post-AI cumulative downloads for Successful post-cutoff libraries. In descriptive terms, pre-cutoff libraries average roughly three times more weekly downloads than post-cutoff libraries in the post-ChatGPT period, growing to approximately five times by early 2026, compared to a modest 28\% baseline difference before ChatGPT. The finding survives placebo testing, is robust across bandwidths, and is amplified in the outcome most directly attributable to LLM-generated code (GitHub imports, $+26.8$, Table~\ref{tab:github-rdd}). The absence of any comparable discontinuity in the 2018--2020 placebo cohorts and the activation pattern (the gap emerging only after November 2022) are consistent with the proposed mechanism.
...
Additional threats include the exact placement of the cutoff within September 2021 (addressed by the 9-week donut), the concurrent deployment of GitHub Copilot (which shares the same approximate knowledge boundary, making the finding a property of the model ecosystem rather than a single product), general platform dynamics (addressed by the 2:1 GitHub-to-PyPI ratio), and retrieval-augmented generation capabilities (which should erode the gap if active, yet Figure~\ref{fig:long-horizon} shows it widening through 2026). These threats are discussed in detail in Appendix~\ref{app:threats}; none alters the main conclusion.

\section{The Systemic Wall Interpretation}\label{sec:disc-systemic}
The mechanism moderation test found no significant heterogeneity between High and Low AI exposure libraries ($p = 0.179$ for 52-week downloads; $p = 0.505$ for baseline-adjusted post-AI downloads). This suggests the Knowledge Wall operates as a broad ecosystem effect. LLMs serve as a general discovery and documentation interface; exclusion from this interface reduces discoverability across the entire software stack, not just in AI-heavy contexts.
...
Key limitations include running variable measurement error (PyPI registration lags bias estimates conservatively), matching coverage (the 5.3\% GitHub match rate over-represents prominent libraries), the endogeneity of the AI exposure moderator, and the absence of family-wise error correction across multiple outcomes and tiers. The baseline-adjusted Diff-in-RDD for the Successful tier ($p = 0.002$) is designated as the main specification \textit{ex ante}; the Broad-tier estimate ($p = 0.001$) is comparably significant after baseline adjustment. The causal claim rests on the parallel trends assumption, which is supported by placebo stability but remains fundamentally untestable. Detailed discussion of multiple testing and secular growth threats is provided in Appendix~\ref{app:scope}.
...
The answer to the research question is affirmative. Libraries released just after the cutoff, excluded from the dominant LLMs' parametric knowledge, accumulate significantly fewer downloads and code imports once those models reach mass deployment. Before ChatGPT, the two groups differed by roughly 28\% in weekly downloads, a modest seasonal baseline; by early 2026, pre-cutoff libraries average approximately five times more weekly downloads, and the gap continues to widen. The baseline-adjusted Diff-in-RDD estimate for libraries that demonstrated pre-AI quality (at least 500 downloads within 26 weeks, measured entirely before ChatGPT) is $-7.3$ ($p = 0.002$). The suppression is approximately twice as large in GitHub code imports as in aggregate PyPI downloads, consistent with LLM-steered code generation as the operative channel. Three placebo cohorts show no comparable discontinuity, and the gap activated only after November 2022. The suppression is concentrated in the Broad and Successful tiers; libraries with the highest pre-ChatGPT visibility (Superstar tier, at least 1,000 downloads) show no significant penalty ($p = 0.473$), consistent with very high-visibility libraries maintaining adoption through channels independent of LLM recommendation.
...
The two-sided empirical $p$-value is 0.344 for the raw RDD estimate at the September 2021 cutoff. That is, the raw 52-week download discontinuity at the true cutoff is not statistically unusual relative to discontinuities observed at arbitrary placebo dates. Figure~\ref{fig:permutation} shows the distribution of placebo RDD coefficients with the true estimate marked.
...
The GitHub implementation estimates are not subject to a Diff-in-RDD counterpart and are more sensitive to bandwidth choice than the PyPI Diff-in-RDD. At $h = 13$ weeks the post-AI import discontinuity is $26.8$ ($p = 0.014$); at $h = 18$ weeks it falls to $2.9$ ($p = 0.129$). These estimates are therefore treated as descriptive evidence consistent with the channel interpretation, not as independently robustified causal estimates.

Replacing the asymmetric 9-week donut with a symmetric $\pm 4$-week exclusion produces qualitatively consistent results. The effect remains robust (e.g., $Estimate = -4.35, p < 0.001$ for the Successful tier with a symmetric donut), suggesting the Knowledge Wall is not an artifact of the specific exclusion window.

% ------------------------------------------------------------------
\chapter{Mechanism Moderation Test}\label{app:mechanism}

A direct mechanism test examines whether suppression is heterogeneous by AI exposure intensity. The specification is:
$$Y_i = \alpha + \tau D_i + \gamma \cdot \text{HighAI}_i + \delta \cdot (D_i \times \text{HighAI}_i) + f(X_i) + \varepsilon_i$$
where $\text{HighAI}_i$ indicates libraries above the median of the commit-level AI score (\texttt{avg\_ai\_score\_52wk}) in the GitHub-matched subsample. The coefficient of interest is $\delta$, representing the differential suppression effect for libraries frequently found in AI-generated code.
...
Neither interaction is statistically significant. With $N = 1{,}303$ and approximately 650 observations per AI-exposure group, the interacted split-sample RDD is almost certainly underpowered to detect meaningful moderation. The null result should be interpreted primarily as reflecting insufficient statistical power rather than as positive evidence for a systemic effect. The data cannot distinguish between a broad ecosystem-wide penalty and a targeted effect on AI-mediated usage that the test lacks the power to detect.

An additional caveat: the AI-exposure moderator (\texttt{avg\_ai\_score\_52wk}) is measured post-treatment and may be endogenous to the suppression itself. A library suppressed by the Knowledge Wall will attract fewer imports overall, and specifically fewer AI-generated imports, because it is less known to LLMs. The moderator therefore partly reflects treatment, limiting causal interpretation of the interaction.

% ------------------------------------------------------------------
\chapter{Raw Long-Horizon Trajectory}\label{app:raw-trajectory}

Figure~\ref{fig:raw-trajectory} shows the raw (unnormalized) weekly average PyPI downloads for the July 2021 (pre-cutoff) and October 2021 (post-cutoff) cohorts, complementing the normalized ratio plot in the main text (Figure~\ref{fig:long-horizon}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{results/figures/long_horizon_trajectory_pypi.png}
\caption{Weekly average PyPI downloads for the July 2021 (pre-cutoff) and October 2021 (post-cutoff) cohorts, from release through January 2026 (log scale). Dashed vertical lines mark ChatGPT release (November 2022) and GPT-4 release (March 2023). The gap between cohorts opens after ChatGPT and persists through the end of the observation window, with no evidence of catch-up.}\label{fig:raw-trajectory}
\end{figure}
