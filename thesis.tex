...
The analysis reveals three findings. First, post-cutoff libraries suffer a significant diffusion penalty once LLMs become widely available: before ChatGPT, libraries on either side of the cutoff differed by roughly 28\% in weekly downloads (a seasonal baseline); by early 2026, pre-cutoff libraries average approximately five times more weekly downloads, and the gap continues to widen. The Diff-in-RDD estimate confirms that this suppression is statistically significant ($p = 0.008$) among libraries that achieved at least 500 downloads in their first 26 weeks; post-cutoff libraries were more successful before ChatGPT, making the reversal conservative. Second, the penalty is roughly twice as large in GitHub code imports as in PyPI downloads, consistent with LLM-steered code generation as the operative channel. Third, the gap was absent in all placebo years and emerged only after November 2022.
...
\multicolumn{6}{l}{\textit{Panel A: Pre-ChatGPT baseline (52-week downloads)}} \\
Successful (min 500) & 52-week Downloads & $-3.724$ & 1.669 & 0.026 & 75,313 \\[4pt]
\multicolumn{6}{l}{\textit{Panel B: Post-ChatGPT activation (post-AI downloads)}} \\
...
The excess jump is significantly negative for both the Broad ($-9.1$ log points, $p = 0.039$) and Successful ($-12.8$ log points, $p = 0.008$) tiers. In descriptive terms, libraries narrowly included in the training data average roughly three times more weekly PyPI downloads than narrowly excluded libraries in the post-ChatGPT period (a ratio that grows to approximately 5:1 by early 2026), compared to a modest pre-ChatGPT baseline difference of about 28\%.\footnote{These descriptive ratios compare mean weekly downloads for libraries released in the four weeks before the donut to those released in the four weeks after the cutoff; the Diff-in-RDD log-point estimates control for seasonal patterns via the placebo-year baseline.}

The 52-week outcome provides a pre-ChatGPT baseline. For nearly all libraries within the $h = 13$ bandwidth, the 52-week accumulation window closes before November 2022; libraries at $\text{dist} = +1$ (the nearest post-cutoff observation) have their 52-week window end in early October 2022, still eight weeks before ChatGPT. The 52-week Diff-in-RDD for the Successful tier ($-3.7$ log points, $p = 0.026$; Panel~A of Table~\ref{tab:diffrdd}) therefore reflects a pre-existing seasonal difference: the typical autumn advantage for post-September libraries was 3.7 log points smaller in 2021 than in placebo years, even before LLMs became widely available. This baseline makes the post-AI finding more conservative: the incremental penalty attributable to ChatGPT deployment is the post-AI excess ($-12.8$) net of the pre-existing baseline ($-3.7$), approximately $-9.1$ log points.
...
The core result is a $-12.8$ log point Diff-in-RDD penalty in post-AI cumulative downloads for Successful post-cutoff libraries ($p = 0.008$). In descriptive terms, pre-cutoff libraries average roughly three times more weekly downloads than post-cutoff libraries in the post-ChatGPT period, growing to approximately five times by early 2026, compared to a modest 28\% baseline difference before ChatGPT. The finding survives placebo testing, is robust across bandwidths, and is amplified in the outcome most directly attributable to LLM-generated code (GitHub imports, $+26.8$ log points). The absence of any comparable discontinuity in the 2018--2020 placebo cohorts and the activation pattern (the gap emerging only after November 2022) are consistent with the proposed mechanism.
...
Key limitations include running variable measurement error (PyPI registration lags bias estimates conservatively), matching coverage (the 5.3\% GitHub match rate over-represents prominent libraries), the endogeneity of the AI exposure moderator, and the absence of family-wise error correction across multiple outcomes and tiers. The Diff-in-RDD for the Successful tier ($p = 0.008$) is designated as the main specification \textit{ex ante}; marginal results (e.g., the Broad-tier Diff-in-RDD at $p = 0.039$) should be interpreted with additional caution. The causal claim rests on the parallel trends assumption, which is supported by placebo stability but remains fundamentally untestable. Detailed discussion of multiple testing and secular growth threats is provided in Appendix~\ref{app:scope}.
...
The answer to the research question is affirmative. Libraries released just after the cutoff, excluded from the dominant LLMs' parametric knowledge, accumulate significantly fewer downloads and code imports once those models reach mass deployment. Before ChatGPT, the two groups differed by roughly 28\% in weekly downloads, a modest seasonal baseline; by early 2026, pre-cutoff libraries average approximately five times more weekly downloads, and the gap continues to widen. The Diff-in-RDD estimate for libraries that demonstrated pre-AI quality (at least 500 downloads within 26 weeks, measured entirely before ChatGPT) is $-12.8$ log points ($p = 0.008$). The suppression is approximately twice as large in GitHub code imports as in aggregate PyPI downloads, consistent with LLM-steered code generation as the operative channel. Three placebo cohorts show no comparable discontinuity, and the gap activated only after November 2022. The suppression is concentrated in the Broad and Successful tiers; libraries with the highest pre-ChatGPT visibility (Superstar tier, at least 1,000 downloads) show no significant penalty ($p = 0.261$), consistent with very high-visibility libraries maintaining adoption through channels independent of LLM recommendation.
...
\begin{tablenotes}
\small
\item \textit{Notes:} Interacted WLS with week-clustered standard errors. Triangular kernel, $h = 26$ weeks, 9-week asymmetric donut. Bandwidth differs from the main Diff-in-RDD ($h = 13$) to retain sufficient observations for the split-sample design.
\end{tablenotes}
\end{threeparttable}
\end{table}
