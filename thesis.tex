...
The analysis reveals three distinct findings: first, post-cutoff libraries suffer a significant diffusion penalty once LLMs are publicly available: the Diff-in-RDD estimate is $-12.8$ log points for libraries that achieved at least 500 downloads in their first 26 weeks ($p = 0.008$). Post-cutoff libraries were more successful before ChatGPT, thus making the reversal conservative. Second, the penalty is roughly twice as large in GitHub imports ($26.8$ log points) as in PyPI downloads ($13.1$ log points), consistent with LLM-supported code generation getting more and more adopted. Third, the gap was absent in all placebo years and emerged only after November 2022, and as of January 2026, no catch-up is observed.
