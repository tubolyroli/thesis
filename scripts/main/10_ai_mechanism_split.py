...
def main():
    ...
    for out in outcomes:
        # Baseline covariate: skip when outcome is the baseline itself
        cov = ["total_downloads_52wk"] if out != "total_downloads_52wk" else None
        ...
        for group_val, label in labels.items():
            ...
            # Baseline-adjusted
            if cov:
                res_adj = run_local_linear_rdd(sub, out, h=26, donut_weeks=DONUT_WEEKS, label=f"Appendix: {label} (Adj)", cluster_col="dist_to_cutoff", covs_cols=cov)
                results.append(res_adj)
    ...
    for out in outcomes:
        ...
        for adjusted in [False, True]:
            ...
            # Build formula
            if adjusted:
                sub_interact["log_baseline"] = np.log1p(sub_interact["total_downloads_52wk"])
                formula = "log_y ~ log_baseline + treated * high_ai + x * treated * high_ai"
            else:
                formula = "log_y ~ treated * high_ai + x * treated * high_ai"
